...
### MCP Server with CLI Binary

Alternatively, you can integrate the tool with MCP by executing the pre-built binary directly, without using the Go API. This approach avoids Go dependencies and uses the standalone binary.
...
Ensure the `tls-cert-chain-resolver` binary is in your system PATH, or update the `exec.CommandContext` call to use the full binary path.

#### Example Usage in Claude Desktop

Ask Claude to resolve certificate chains:
...
The tool executes the binary and returns the resolved certificate chain.
