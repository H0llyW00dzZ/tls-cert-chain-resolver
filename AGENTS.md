...
- [9. Modifying Generated Code](#9-modifying-generated-code)
...
**Test all**: `go test -v ./... 2>&1 | cat` or `make test` (note: `make test` includes race detector and coverage by default)  
...
#### 9. Modifying Generated Code

**❌ Bad: Editing files with "Code generated by go generate" header**

```go
// ❌ BAD - Editing src/mcp-server/resources.go
...
**✅ Good: Update configuration or templates instead**

```bash
# ✅ GOOD - Edit configuration or templates
edit("tools/codegen/config/resources.json", ...)
edit("tools/codegen/templates/resources.go.tmpl", ...)
...
- **LRU cache testing**: Test O(1) LRU implementation with hashmap and doubly-linked list, including access order maintenance, eviction correctness, concurrent access, edge cases, and resource leak prevention. See `src/internal/x509/chain/lru_test.go` for comprehensive LRU test coverage.
