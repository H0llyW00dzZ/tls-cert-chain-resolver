...
- `golang.org/x/crypto` (via Go 1.25.4) - Standard crypto updates leveraged in recent releases
  ...
  **JSON-RPC Utilities**: Use `src/internal/helper/jsonrpc/` for JSON-RPC 2.0 canonicalization and normalization (lowercase keys, ID handling) when implementing MCP transports. Prefer using defined structs (e.g., `jsonRPCResponse`, `jsonRPCError`) over raw maps for type-safe message handling.
